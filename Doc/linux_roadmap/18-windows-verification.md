...
| Re-confirm `alienfx-tools.sln` still builds green after M2c's enumeration/detection rewrite | M2c | Same as the recurring "ongoing" row below, called out separately here because M2c is the first M2 sub-milestone to *edit* code M2a added inside the Linux `#else` arm (the enumeration/detection stub), rather than only adding new files/arms — worth a fresh check that the Windows `#ifdef _WIN32` arm of `AlienFXProbeDevice`/`AlienFXInitialize` is untouched | See "How to verify a Windows build" below | **Open** |
