...
| Capture golden byte vectors: instrument `PrepareAndSend` (`AlienFX-SDK/AlienFX_SDK/AlienFX_SDK.cpp`) to dump `buffer[0..length)` before `HidD_SetFeature`/`HidD_SetOutputReport`/etc., one call per API version × command type | M2a (upgrade) | Needs the existing Windows build talking to real HID devices via `HidD_*`/`WriteFile` | See [16](16-testing-and-validation.md)'s golden-vector section; write output using the hex format in `tests/README.md` | **Open — no longer a blocker for M2a.** M2a's exit criteria use `source-derived`/`hand-derived` vectors instead (see [17](17-milestones.md)'s M2 split and [16](16-testing-and-validation.md)'s three-tier table); this row is now an **upgrade path** — performing it replaces a vector file's contents in place with `origin=hardware-captured` provenance, it does not gate any milestone. |
...
| Re-confirm `alienfx-tools.sln` still builds green after M2c's enumeration/detection rewrite | M2c | Same as the recurring "ongoing" row below, called out separately here because M2c is the first M2 sub-milestone to *edit* code M2a added inside the Linux `#else` arm (the enumeration/detection stub), rather than only adding new files/arms — worth a fresh check that the Windows `#ifdef _WIN32` arm of `AlienFXProbeDevice`/`AlienFXInitialize` is untouched | See "How to verify a Windows build" below | **Open.** M2a–M2b's exact-diff proof (`git diff -U0 ... \| grep '^-[^-]'` staying empty) does not extend to M2c — this is the first M2 sub-milestone that isn't purely additive, which is why this row exists at all. In its place, a mechanical proof was run in lieu of an actual Windows build: `AlienFX_SDK.cpp`'s Windows `#ifdef _WIN32` arm (`AlienFXProbeDevice`/`AlienFXInitialize`, up to their `#else`) is confirmed byte-for-byte identical to pre-M2c HEAD via `awk '/^#ifdef _WIN32$/{n++;next} n==2{if(/^#else$/)exit;print}'` applied to both revisions, and `git diff --stat HEAD -- '*.sln' '*.vcxproj' '*.vcxproj.*' '*.vcxitems' '*.props' '*.targets'` is empty (no `Directory.Build.*` file added either). This proves MSBuild's input set for the Windows arm is unchanged, but is not a substitute for an actual `alienfx-tools.sln` build — the row stays Open until someone with Windows access performs one. |
